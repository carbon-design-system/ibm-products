...
const HeaderRow = (datagridState, headRef, headerGroup) => {
    ...
    if (e.detail === 2) {
      ...
      datagridState.rows.map((row) => {
        ...
        if (
          ...
          cellWidths.push(cell.firstChild.scrollWidth, headerLabel.scrollWidth);
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const colDefaultWidth = datagridState.allColumns.filter(
      (column) => column.id === resizingCol
    );
    if (resizingCol !== null && selectedHeader.length > 0) {
      ...
      if (
        ...
        setDragStopped(true);
        columnResizing.columnWidth = isNaN(resizingCol)
          ? colDefaultWidth[0]?.width
