...
createOverflowHandler({
  container, // The container element that holds the items
  maxVisibleItems, // Maximum number of visible items. If provided, only this number of items will be shown.
  onChange: (visibleItems, hiddenItems) => {}, // Callback function invoked when the visible and hidden items change.
  dimension // The dimension to consider for overflow calculations. Defaults to 'width'.
});
```
