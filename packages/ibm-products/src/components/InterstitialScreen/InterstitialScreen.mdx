...
Footer. `InterstitialScreen` acting as a container and `InterstitialScreen.Header`,
`InterstitialScreen.Body`, `InterstitialScreen.Footer` are static properties of the
`InterstitialScreen` component, effectively acting as sub-components. All related
parts of the screen, the Header, Body and Footer are grouped under a single logical
component called `InterstitialScreen`. You can mix and match subcomponents or even omit
some.
