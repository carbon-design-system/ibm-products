...
import { StepsContext, StepNumberContext } from './EditTearsheet';
import { pkg } from '../../settings';
import pconsole from '../../global/js/utils/pconsole';
import { usePreviousValue, useRetrieveStepData } from '../../global/js/hooks';

const componentName = 'EditTearsheetStep';
const blockClass = `${pkg.prefix}--tearsheet-edit__step`;
...
export let EditTearsheetStep = forwardRef(
  (
    {
    ...
    useEffect(() => {
      if (
        ...
        stepsContext?.setOnNext(stepsContext?.currentStep);
...
EditTearsheetStep = pkg.checkComponentEnabled(EditTearsheetStep, componentName);

EditTearsheetStep.propTypes = {
  /**
   ...
   * This function can _optionally_ return a promise that is either resolved or rejected and the EditTearsheet will handle the submitting state of the next button.
   */
