...
textarea, number, time, and date—as well as a custom type that allows users to render their own custom components.
 ...
 ### inputConfig
**Input types** <br/>
 option : Support single select as well was multi select. Multi-select will be activated for one-of operator or user preferred multi select custom operator.
...
For input types like text, textarea, number, date, and time, the component uses the corresponding Carbon components and supports standard Carbon props.

The date range picker can be enabled when using the '`between`' operator or user preferred multiselect custom operator. 
Eg: you can customise the date format and locale by setting `properties[i].config.dateFormat` and `properties[i].config.locale`, respectively.
