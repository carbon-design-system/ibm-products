...
<cds-modal size="sm" ?open="${this._open}" prevent-close>
  ...
  <cds-modal-body>
    ...
    >
    <ul class="no-bullets">
      <li><cds-link> Route1_name ${launchSVG} </cds-link></li>
      <li><cds-link> Hpt-392-ser ${launchSVG} </cds-link></li>
      <li><cds-link> MKtps_02_094 ${launchSVG} </cds-link></li>
    </ul>

    <p>
      Note - the following selected items cannot be deleted:
      <cds-link>Gigstore-0034</cds-link>, <cds-link>Gigstore-0058</cds-link>
    </p>
  </cds-modal-body>
  <cds-modal-footer>
    <cds-modal-footer-button kind="secondary" @click="${this._close}"
