...
import editFullPageAnatomy from './storybook_assets/edit-full-page-1.png';
...
- The main value of the object is its settings and it needs a “home” (i.e. its
  ...
  mode, and the save changes/cancel changes buttons are shown in a disabled
  state until a change is detected.
...
Anatomy of EditFullPage

<img src={editFullPageAnatomy} />

1. **Editable content**: Everything in the page is editable and shown in the
   editable state.

2. **Save/cancel buttons**: Cancel changes and save changes buttons are always
   shown as disabled until a change is detected on the page. Note that “changes”
   is added to the button names in this case because the title of the page does
   not have “Edit” in it. In a toggle use case where the content on the page was
   static until the user turned on an edit mode, the title of the page would
   change to “Edit [object]” and the buttons would be labeled “Cancel” and
   “Save”.
