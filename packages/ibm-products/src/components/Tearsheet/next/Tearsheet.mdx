...
export const customFunctionArr = [
  [
    `const decorator=undefined,isFlush=undefined,influencerWidth=undefined,keepMounted=undefined,summaryContentWidth=undefined,verticalGap=undefined,variant=['wide', 'wide', 'wide'],hideCloseButton=undefined,disableHeaderCollapse=undefined,closeIconDescription=undefined, context={};
...
of **40px**. In this reduced state, action buttons inside
`Tearsheet.HeaderActionItem` must switch to `size="xs"` to fit the available
space.

Use the `onHeaderCollapse` callback on `Tearsheet.Header` to react to the change
  ...
  additionalActions={[
    {
      ...
      onClick: () =>
        window.open(
          'https://stackblitz.com/github/carbon-design-system/ibm-products/tree/main/packages/ibm-products/src/patterns/TearsheetWithSteps/example',
          '_blank'
        ),
    },
  ]}
