...
import { arrayOf, func, oneOf, oneOfType, shape, string } from 'prop-types';
import {
...
const Links = ({ links: linksProp }) => {
  const links = Array.isArray(linksProp) ? linksProp : [linksProp];
  return (
    <ul>
      {links.map(({ text, url, target = '_top' }) => (
        <li {...{ className: `${prefix}__link`, key: `${text}:${url}` }}>
