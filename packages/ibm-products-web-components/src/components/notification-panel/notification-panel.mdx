...
`c4p-notification-footer` component can be used, which comes with built-in
styling.
...
areas. These sections are intended to contain `c4p-notification` components.
Each `c4p-notification` supports two named slots: title and description,
...
>
  ...
  <c4p-notification-footer
    ...
    @c4p-notification-view-all="${action(`Clicked"
    View
    All`)}
    @c4p-notification-settings="${action(`Clicked"
    Settings`)}
  ></c4p-notification-footer>
</c4p-notification-panel>
```
